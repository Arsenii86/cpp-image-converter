...
bool SaveJPEG(const Path& file, const Image& image) {     
    ...
    std::vector<JSAMPLE> image_buffer(3*image.GetWidth()); 
    ...
    while (cinfo.next_scanline < cinfo.image_height) {        
        ...
        for (int x = 0; x < image.GetWidth(); x++){
            image_buffer.push_back(static_cast<JSAMPLE>(line[x].r));
            image_buffer.push_back(static_cast<JSAMPLE>(line[x].g));
            image_buffer.push_back(static_cast<JSAMPLE>(line[x].b));
        }
